...
{
    [ApiController]
    [Route("/person-genders")]
    public class PersonGenderController : ControllerBase
    {
        private readonly IPersonGenderService service;

        public PersonGenderController(IPersonGenderService service)
        {
        ...
        public async Task<IActionResult> Create([FromBody] PersonGenderRequest request)
        {
        ...
        [HttpPut("{id}")]
        [Authorize(Roles = ConstantHelper.Role.ADMIN)]
        public async Task<IActionResult> Update([FromBody] PersonGenderRequest request, [FromRoute] int id)
        {
